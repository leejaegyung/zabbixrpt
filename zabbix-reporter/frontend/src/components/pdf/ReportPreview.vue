        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="item in page" :key="getItemId(item)">
            ...
            <td class="py-3 px-4 text-sm text-center">
              <span v-if="getHostStatusInfo(item) === 'DISABLED'" class="inline-flex items-center justify-center h-6 leading-none px-3 rounded-full text-xs font-bold bg-gray-100 text-gray-500 border border-gray-200">비활성</span>
              <span v-else-if="getHostStatusInfo(item) === 'NORMAL'" class="inline-flex items-center justify-center h-6 leading-none px-3 rounded-full text-xs font-bold bg-green-50 text-green-700 border border-green-200">ZBX 정상</span>
              <span v-else-if="getHostStatusInfo(item) === 'ERROR'" class="inline-flex items-center justify-center h-6 leading-none px-3 rounded-full text-xs font-bold bg-red-50 text-red-700 border border-red-200">ZBX 오류</span>
              <span v-else class="inline-flex items-center justify-center h-6 leading-none px-3 rounded-full text-xs font-bold bg-gray-50 text-gray-600 border border-gray-200">상태 대기</span>
            </td>
          </tr>
        </tbody>
      </table>
      ...
      <table class="w-full text-left border-collapse bg-white rounded-lg overflow-hidden">
        ...
        <tbody class="divide-y divide-gray-100">
          <tr v-for="item in page" :key="getItemId(item)">
            ...
            <td class="py-3 px-4 text-sm"><span class="inline-flex items-center justify-center h-6 leading-none px-2.5 rounded-md text-xs font-bold" :class="getSeverityColor(item.severity)">{{ getSeverityText(item.severity) }}</span></td>
