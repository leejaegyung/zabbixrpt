        ...
        <div v-if="a.storageForecast.length > 0" class="flex flex-col gap-2.5">
          <div v-for="(item, idx) in a.storageForecast" :key="idx" class="p-2.5" :class="c.cell">
            <div class="flex justify-between items-center mb-1.5 gap-2">
              ...
              <span v-if="item.prediction.status === 'critical'" class="text-[10px] px-2 py-0.5 font-bold uppercase tracking-cta bg-rosso text-ink shrink-0">Critical</span>
              <span v-else-if="item.prediction.status === 'warning'" class="text-[10px] px-2 py-0.5 font-bold uppercase tracking-cta shrink-0" :class="pdf ? 'bg-orange-100 text-orange-700' : 'text-sig-warning border border-sig-warning/40'">D-{{ item.prediction.daysLeft }}</span>
              <span v-else class="text-[10px] px-2 py-0.5 font-bold uppercase tracking-cta shrink-0" :class="pdf ? 'bg-green-50 text-green-700' : 'text-sig-success border border-sig-success/40'">Stable</span>
            </div>
